...
import { TransferIntransitDetailsHttpDataService } from './transfer-intransit-details-http-data.service';
var itemListArray: any = [];

@Component({
  selector: 'app-transfer-intransit-details',
  templateUrl: './transfer-intransit-details.component.html',
  styleUrls: ['./transfer-intransit-details.component.css']
})

export class TransferIntransitDetailsComponent implements OnInit {
  @ViewChild("gridContainer") gridContainer: DxDataGridComponent;
  @ViewChild("datagridforitems") datagridforitems: DxDataGridComponent;

  TRNumber: String = UtilsForGlobalData.retrieveLocalStorageKey('TRINumber');
  transHeader: any;
  ...
  columns1 = [
    ...
    { title: "QuantitySent", dataKey: "QuantitySent", width: 40 },
  ];
  columns2 = [
    { title: "SNo", dataKey: "SnNo", width: 90 },
  ...
  constructor(
    private httpDataService: TransferIntransitDetailsHttpDataService,
      ...
      .subscribe(openTransHeader => {
        ...
        if (this.transHeader["FromLocationCode"]) {
          if (this.transHeader["ToLocationCode"]) {
            if (this.transHeader["InTransitCode"]) {
    ...
    this.dataSource = new CustomStore({
      ...
      insert: function (values) {
        ...
        if (thisComponent.allowAdding ? values["ItemCode"] : false) {
          if (values["QuantitySent"] > 0) {
            for (var index = 0; index < itemListArray.length; ++index) {
              if (itemListArray[index].ItemCode == values["ItemCode"] && itemListArray[index].LotNumber == values["LotNo"]) {
                if (Number(values["QuantitySent"]) <= Number(itemListArray[index].Stock)) {
                  ...
                  devru.reject("Quantity is Greater than Stock Avalable( " + parseFloat(itemListArray[index].Stock).toFixed(2) + " )!, Quantity Cannot be Exceeded!!");
                }
                break;
              }
            }
          ...
          devru.reject("Select the From/Intransit/To Location Code!!! , Data Required");
        }
        return devru.promise();
      }
    });
  ...
  postTransfer() {
    this.httpDataService.btnSend_clickHandler2(['', 'PostToIntransit', this.TRNumber,
      UtilsForGlobalData.getUserId()]).subscribe(btnSend_clickHandler2 => {
        if (btnSend_clickHandler2[0] == 'DONE') {
          ...
          this.router.navigate(['/warehouse/transfer-intransit-list']);
        } else {
  ...
  generateStdPDF(printHeader, printLines, columHeader, title) {
    ...
    doc.textAlign("" + this.formatNumber(printHeader.TotalQuantitySent), { align: "right-align" }, rightcol2, startY);
