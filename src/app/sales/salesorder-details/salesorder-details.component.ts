...
import { SalesorderDetailsHttpDataService } from './salesorder-details-http-data.service';
...
@Component({
  selector: 'app-salesorder-details',
  templateUrl: './salesorder-details.component.html',
  styleUrls: ['./salesorder-details.component.css']
})
export class SalesorderDetailsComponent implements OnInit {
  ...
  SONumber: string = UtilsForGlobalData.retrieveLocalStorageKey('SONumber');
  ...
  constructor(
    private httpDataService: SalesorderDetailsHttpDataService,
  ...
  ngOnInit() {
    ...
    this.httpDataService.GetSOItemListWithInventory(['',
      this.SONumber, '1']).subscribe(callData3 => {
        this.itemArray = {
          paginate: true,
          pageSize: 20,
          loadMode: "raw",
    ...
    this.httpDataService.getServiceItems(['']).subscribe(getSItem => {
      this.serviceitemArray = <String[]>getSItem;
    });
    ...
    this.dataSource.store = new CustomStore({
      key: ["LineNo", "lineType", "ItemCode", "Description", "UOM", "UnitPrice", "UnitPriceFCY", "Quantity",
        "Location", "LineDiscountAmount", "FOC", "LineDiscountAmtText", "CostIncVAT", "VatAmount"],
      ...
      remove: function (key) {
        var devru = $.Deferred();
        if (Number(thisComponent.Customer["InvDiscountAmount"]) == 0) {
          if (key["lineType"] == 'DEPOSIT') {
            thisComponent.httpDataService.deleteLineProcedure(["",
              thisComponent.SONumber, key["LineNo"],
              key["ItemCode"]]).subscribe(dataStatus => {
      ...
      insert: function (values) {
        var devru = $.Deferred();
        if (values["lineType"] ? values["ItemCode"] ? true : false : false) {
          if (values["lineType"] == 'ITEM') {
            ...
            thisComponent.httpDataService.CompoundDiscountP(["", GAmount, values["LineDiscountAmount"]])
              .subscribe(dataDiscLines => {
                ...
                thisComponent.httpDataService.tl_itemDoubleClickHandler(["",
                  thisComponent.SONumber,
                  values["ItemCode"],
                  values["UOM"],
                  values["UnitPrice"],
                  thisComponent.Customer["AmtIncvat"] == true ? 'Yes' : 'No',
                  values["Quantity"],
                  disc,
                  values["LineDiscountAmount"],
                  thisComponent.Customer["OrderDate"],
                  thisComponent.Customer["LocationCode"]]).subscribe(data => {
                    if (data[0] == 'DONE') {
                      devru.resolve(data);
                    } else {
                      devru.reject("Error while Inserting the Lines with ItemCode: " + values["ItemCode"] + ", Error Status Code is " + data[0]);
                    }
                  });
              });
          }
            ...
            thisComponent.httpDataService.addServiceLine(["", thisComponent.SONumber,
              values["ItemCode"],
              values["UnitPrice"]]).subscribe(dataStatus => {
                if (dataStatus[0] == 'DONE') {
                  devru.resolve(dataStatus);
                } else {
            ...
            thisComponent.httpDataService.insertDeposit(["", thisComponent.SONumber,
        ...
        if (Number(thisComponent.Customer["InvDiscountAmount"]) == 0) {
          ...
          thisComponent.httpDataService.CompoundDiscountP(["", GAmount, getUpdateValues(key, newValues, "LineDiscountAmount")])
            .subscribe(dataDiscLines => {
              ...
              thisComponent.httpDataService.btnSave_clickHandler(["", thisComponent.SONumber,
                ...
                getUpdateValues(key, newValues, "FOC") ? getUpdateValues(key, newValues, "FOC") : "No"]).subscribe(data => {
                  if (data[0] == 'DONE') {
                    devru.resolve(data);
                  } else {
                    devru.reject("Error while Updating the Lines with ItemCode: " + getUpdateValues(key, newValues, "ItemCode") + ", Error Status Code is " + data[0]);
                  }
                });
            });
        } else {
    ...
    this.httpDataService.getCompanyInfo().subscribe(getCompany => {
      ...
      var thisC = this;
      img.onload = function () {
        thisC.companyHeader.CompanyLogoWidth = parseInt(img.width.toString());
        thisC.companyHeader.CompanyLogoHeight = parseInt(img.height.toString());
      }
  ...
  calculateForSummary() {
    this.httpDataService.openHeader(["", this.SONumber]).subscribe(dataHeader => {
      ...
      this.InvoiceTotal = this.Customer["InvDiscountAmount"] == null ? 0 : this.Customer["InvDiscountAmount"];
      this.lineTotal = this.Customer["Amount"] == null ? 0 : Number(this.Customer["Amount"]);
      this.DiscountPerc = this.Customer["InvoiceCompoundDiscount"];
      this.SubTotal = Number(this.lineTotal) - Number(this.InvoiceTotal);
      this.amtincvatvalue = this.Customer["AmountIncludingVAT"] == null ? 0 : Number(this.Customer["AmountIncludingVAT"]);
      this.httpDataService.getTotalLinesDiscAmt(['',
        this.SONumber]).subscribe(getTotalLinesDiscAmt => {
          ...
          this.totalInvoiceDisocunt = Number(Number(this.TotalLineDiscountAmount) + Number(this.Customer["InvDiscountAmount"])).toFixed(2);
        });
      ...
      this.TotalDeposit = this.Customer["DepositAmount"] == null ? 0 : this.Customer["DepositAmount"];
      if (this.Customer["AmtIncvat"] == 'Yes') {
        this.Customer["AmtIncvat"] = true;
      } else {
        this.Customer["AmtIncvat"] = false;
      }
      if (this.Customer["TaxLiable"] == 'Yes') {
        this.Customer["TaxLiable"] = true;
      } else {
        this.Customer["TaxLiable"] = false;
      }
      ...
      if (Object.keys(this.itemLookupSuggestionDataGrid).length == 0) {
        this.getLookupForDataGrid(this.Customer["BilltoCustomerNo"], this.Customer["OrderDate"]);
      }
    });
  }
  ...
  setTheSettingStatus() {
    if (this.Customer['FlowResult'] == 'Approved') {
      ...
      this.SalesOrderOperations = ['Print Order', 'Convert to PO'];
    }
    else if (this.Customer['FlowResult'] == 'Rejected') {
      this.SOFlowResult = true;
      this.SalesOrderOperations = ['Print Order'];
    }
    else if (this.Customer['FlowResult'] == 'SENT FOR APPROVAL') {
      this.SOFlowResult = false;
      this.SalesOrderOperations = ['Approved', 'Rejected', 'Print Order'];
    }
    else {
      this.SOFlowResult = false;
      this.SalesOrderOperations = ['SENT FOR APPROVAL', 'Print Order'];
    }
  }
    ...
    if (this.newCustomerDetail["CustomerID"]) {
      this.httpDataService.insertCustomer(["",
        ...
        if (this.customerValue > 0) {
          ...
          this.graphdata(this.newCustomerDetail["CustomerID"]);
        }
        else {
  ...
  graphdata(custCode) {
    this.httpDataService.getGeneralStats(['', custCode])
  ...
  setDiscountValue(newData, value, currentData): void {
    ...
    if (currentData.UnitPrice != undefined && currentData.Quantity != undefined) {
      ...
      this.httpDataService.CompoundDiscountP(["", GAmount, value])
        .subscribe(dataDiscLines => {
          var disc = 0;
          if (dataDiscLines[0] == "invalid value") {
            disc = 0;
  ...
  SalesOrderOperationsGo(userOption) {
    if (userOption == '') {
      ...
      this.httpDataService.getAllReportsSetup(["", "SO"]).subscribe(callData3 => {
        this.reportCustom = callData3;
        this.fetchDataForInvoice();
      });
    } else if (userOption == 'Convert to PO') {
  ...
  onVendorRowClicked(event) {
    this.convertToPO = false;
    this.httpDataService.handleBuyFromLookUpManager(['', this.SONumber, event.data.VendCode,
      UtilsForGlobalData.getUserId()]).subscribe(dataStatus => {
        if (dataStatus[0] == 'DONE') {
          ...
          this.toastr.success("Purchase Order successfully " + dataStatus[1] + " Created");
        } else {
          this.toastr.warning("Purchase Order Creation Failed!!, Error Status Code : " + dataStatus[0]);
        }
      });
  }
  ...
  OnUserActionGo(userOption) {
    if (this.Customer['FlowCompleted'] == 'Yes') {
      ...
      this.toastr.error("Please Add the Items To Sales Order");
    }
              ...
              this.httpDataService.getProcessFlow(["",
                nextSequence.toString()]).subscribe(dataFlow => {
                  if (dataFlow[0]["ResultIsChoice"] == 'No') {
                    ...
                    if (userOption == 'Approved') {
                      if (this.Customer["SalesType"] == 'WOOCOMMERCE') {
                        this.httpDataService.updateOrdertoApproved(["",
                          this.Customer["ExternalDocumentNo"]
                        ]).subscribe(updateOrdertoApproved => {
                          console.log(updateOrdertoApproved);
                          if (updateOrdertoApproved == true) {
                            this.toastr.success("Woocommerce Status Updated!")
                          }
                        });
                      }
                    }
  ...
  onCustomerRowClicked(event) {
    ...
    this.httpDataService.updateCustCode(["", this.SONumber, event.data.CustCode, this.typeOfCustomer,
      UtilsForGlobalData.getUserId()]).subscribe(dataStatus => {
        if (dataStatus[0] == 'DONE') {
          if (this.typeOfCustomer == 'BillTo')
            this.getLookupForDataGrid(event.data.CustCode, this.Customer["OrderDate"]);
        }
  ...
  onBillToCustomerCodeChanged(event) {
    ...
    for (var index = 0; index < json.length; ++index) {
      if (json[index].CustCode == event.value) {
        this.duplicateSalesHeader[0]["BilltoCustomerNo"] = json[index].CustCode;
        this.duplicateSalesHeader[0]["BilltoName"] = json[index].Name;
        this.duplicateSalesHeader[0]["BilltoAddress"] = json[index].Address1;
        this.duplicateSalesHeader[0]["BilltoCountry"] = json[index].Country;
        this.duplicateSalesHeader[0]["PaymentTerm"] = json[index].PaymentTerm;
        this.duplicateSalesHeader[0]["VATID"] = json[index].VATID;
        this.duplicateSalesHeader[0]["BilltoContact"] = json[index].Phone;
        this.duplicateSalesHeader[0]["BillToCustEmail"] = json[index].Email;
        this.Customer["BilltoCustomerNo"] = json[index].CustCode;
        this.Customer["BilltoName"] = json[index].Name;
        this.Customer["BilltoAddress"] = json[index].Address1;
        this.Customer["BilltoCountry"] = json[index].Country;
        this.Customer["PaymentTerm"] = json[index].PaymentTerm;
        this.Customer["VATID"] = json[index].VATID;
        this.Customer["BilltoContact"] = json[index].Phone;
        this.Customer["BillToCustEmail"] = json[index].Email;
        this.httpDataService.updateCustCode(["",
          this.SONumber, event.value, 'BillTo',
          UtilsForGlobalData.getUserId()]).subscribe(callData3 => {
            //this.calculateForSummary();
          });
        break;
  ...
  onShipToCustomerCodeChanged(event) {
    ...
    for (var index = 0; index < json.length; ++index) {
      if (json[index].CustCode == event.value) {
        this.duplicateSalesHeader[0]["ShiptoCode"] = json[index].CustCode;
        this.duplicateSalesHeader[0]["ShiptoName"] = json[index].Name;
        this.duplicateSalesHeader[0]["ShiptoAddress"] = json[index].Address1;
        this.duplicateSalesHeader[0]["ShiptoCountryCode"] = json[index].ShiptoCountryCode;
        this.duplicateSalesHeader[0]["ShiptoContact"] = json[index].Phone;
        this.duplicateSalesHeader[0]["ShipToCustEmail"] = json[index].Email;
        this.Customer["ShiptoCode"] = json[index].CustCode;
        this.Customer["ShiptoName"] = json[index].Name;
        this.Customer["ShiptoAddress"] = json[index].Address1;
        this.Customer["ShiptoCountryCode"] = json[index].ShiptoCountryCode;
        this.Customer["ShiptoContact"] = json[index].Phone;
        this.Customer["ShipToCustEmail"] = json[index].Email;
        this.httpDataService.updateCustCode(["",
  ...
  onDropDownCodeChanged(event, dataField) {
    if (dataField == 'isAmountIncVat') { }
    else if (dataField == 'TaxLiable') { }
    else if (dataField == 'LocationCode') {
      event.value = (event.value == null ? '' : event.value);
      if (this.duplicateSalesHeader[0][dataField] != event.value) {
        this.duplicateSalesHeader[0]["" + dataField] = event.value;
        this.Customer["" + dataField] = event.value;
        this.httpDataService.genUpdateSalesHeader(["",
          this.SONumber, '3', '', event.value, UtilsForGlobalData.getUserId()])
          .subscribe(dataStatus => {
            this.errorHandlingToaster(dataStatus);
          });
      }
  ...
  formSummary_fieldDataChanged(e) {
    if (e.dataField == 'AmtIncvat') {
      ...
      else if (e.dataField == 'OrderDate') {
        this.httpDataService.orderDate_changeHandler(["",
          e.value,
          this.SONumber]).subscribe(dataStatus => {
            if (dataStatus > 0) {
              this.getLookupForDataGrid(this.Customer["BilltoCustomerNo"], e.value);
            }
      ...
      else if (e.dataField == 'TaxLiable') { }
      else {
        this.duplicateSalesHeader[0]['' + e.dataField] = e.value;
        this.httpDataService.genUpdateSalesHeader(["",
          this.SONumber, '4',
          e.dataField, e.value,
          UtilsForGlobalData.getUserId()]).subscribe(dataStatus => {
  ...
  formBilling_fieldDataChanged(e) {
    if (e.dataField != 'AmtIncvat' ? e.dataField != 'TaxLiable' : false) {
  ...
  formContact_fieldDataChanged(e) {
    if ((e.value != undefined || e.value != null) && this.duplicateSalesHeader[0][e.dataField] != e.value) {
  ...
  onCustomerDetailsFieldsChanges(e) {
    if (e.dataField != 'AmtIncvat' ? e.dataField != 'TaxLiable' : false) {
      if ((e.value != undefined || e.value != null) && this.duplicateSalesHeader[0][e.dataField] != e.value) {
        this.duplicateSalesHeader[0]['' + e.dataField] = e.value;
        this.httpDataService.genUpdateSalesHeader(["", this.SONumber, '4',
          e.dataField, e.value,
          UtilsForGlobalData.getUserId()]).subscribe(dataStatus => {
            this.errorHandlingToaster(dataStatus);
          });
  ...
  onEditorPreparing(e) {
    if (e.parentType === "dataRow" && e.dataField === "ItemCode") {
      ...
      e.editorOptions.onValueChanged = function (event) {
        e.component.cellValue(e.row.rowIndex, "UOM", event.value.SalesUOM);
      ...
      /* }  else if (e.parentType === "dataRow" && e.dataField === "LineDiscountAmount") {
        let standardHandler = e.editorOptions.onValueChanged;
        let thisComponent = this;
        e.editorOptions.onValueChanged = function (event) {
          var value = event.value != null ? event.value : 0;
          e.component.cellValue(e.row.rowIndex, "LineDiscountAmtText", value);
          if (e.row.key.UnitPrice != undefined && e.row.key.Quantity != undefined) {
            var GAmount: Number = (e.row.key.Quantity * e.row.key.UnitPrice);
            thisComponent.httpDataService.CompoundDiscountP(["", GAmount, value])
              .subscribe(dataDiscLines => {
                var disc = 0;
                if (dataDiscLines[0] == "invalid value") {
                  disc = 0;
                ...
                var AmountIncludingVAT = Number(GAmount) - disc;
                e.component.cellValue(e.row.rowIndex, "LineDiscountAmount", disc);
                e.component.cellValue(e.row.rowIndex, "AmountIncludingVAT", AmountIncludingVAT);
                e.component.cellValue(e.row.rowIndex, "LineAmount", AmountIncludingVAT);
                thisComponent.gridContainer.instance.saveEditData();
              });
          }
        }*/
    }
  ...
  onUserRowSelect(event, type) {
    if (type == 'ITEM') {
      ...
      this.gridContainer.instance.cellValue(this.rowIndex, "UnitPriceFCY", event.data.UnitPrice);
      ...
      this.gridContainer.instance.cellValue(this.rowIndex, "CostIncVAT", this.Customer["AmtIncvat"] ? 'Yes' : 'No');
  ...
  ItemDeatilsForPopUp(data) {
    this.itemDetails = UtilsForSuggestion.StandartNumberFormat(data.data,
      ["UnitPrice", "Amount", "LineAmount", "AmountIncludingVAT"]);
  ...
  getLookupForDataGrid(PayToVendor: String, DocumentDate: String) {
    this.httpDataService.GetSOItemListWithInventory(["",
      this.SONumber, '1']).subscribe(itemLookupSuggestionDataGrid => {
        ...
        this.httpDataService.getServiceItems(['']).subscribe(serviceLookupSuggestionDataGrid => {
          ...
          for (var i = 0; i < Object.keys(serviceLookupSuggestionDataGrid).length; i++) {
            ...
            itemLookupSuggestionDataGrid[currentIndex + i]["UnitPrice"] = serviceLookupSuggestionDataGrid[i]["UnitPrice"];
            itemLookupSuggestionDataGrid[currentIndex + i]["SalesUOM"] = "UNITS";
          }
          this.httpDataService.getDeposit(['', PayToVendor,
            DocumentDate]).subscribe(depositLookupSuggestionDataGrid => {
              ...
              for (var i = 0; i < Object.keys(depositLookupSuggestionDataGrid).length; i++) {
                ...
                itemLookupSuggestionDataGrid[currentIndex + i]["UnitPrice"] = serviceLookupSuggestionDataGrid[i]["AmountIncludingVAT"];
                itemLookupSuggestionDataGrid[currentIndex + i]["SalesUOM"] = "UNITS";
              }
  ...
  fetchDataForInvoice() {
    if (this.isLinesExist && Object.keys(this.companyHeader).length) {
      this.generateStdPDF(this.Customer, this.printLines, "Sales Order");
    } else {
      this.toastr.warning("Please Add the Item to the Sales Order!");
    }
  }
  ...
  generateStdPDF(printHeader, printLines, title) {
    ...
    doc.save("SalesOrder" + this.SONumber + ".pdf");
  ...
  PrintReportForINDBacK(doc, printHeader, printLines, title) {
    ...
    doc.textAlign(":" + this.companyHeader.StateCode, { align: "left" }, this.pdfFormate.startXcol2Details, tempY);
    doc.textAlign("Sales Ord No", { align: "left" }, this.pdfFormate.rightStartCol1, tempY);
    doc.textAlign(":" + printHeader.DocumentNo, { align: "left" }, this.pdfFormate.startXcol3Details, tempY);
    ...
    doc.textAlign(":" + printHeader.BilltoCity + ", " + printHeader.BilltoPostCode + "-" + printHeader.BilltoCountry, { align: "left" }, this.pdfFormate.startXDetails, tempY);
    doc.textAlign("Contact", { align: "left" }, tempX, tempY += this.pdfFormate.NormalSpacing);
    splitTitle = doc.splitTextToSize(printHeader.BilltoContact + " Ph: " + printHeader.BilltoContactPh, 200);
    tempY -= this.pdfFormate.NormalSpacing;
    for (var i = 0; i < splitTitle.length; i++) {
      doc.textAlign(":" + splitTitle[i], { align: "left" }, this.pdfFormate.startXDetails, tempY += this.pdfFormate.NormalSpacing);
    }
    ...
    doc.textAlign(":" + printHeader.ShiptoCity + ", " + printHeader.ShiptoPostCode + "-" + printHeader.ShiptoCounty, { align: "left" }, this.pdfFormate.startXcol4Details, tempYC);
    doc.textAlign("Contact", { align: "left" }, tempX, tempYC += this.pdfFormate.NormalSpacing);
    splitTitle = doc.splitTextToSize(printHeader.ShiptoContact + " Ph: " + printHeader.ShiptoContactPh, 200);
    tempYC -= this.pdfFormate.NormalSpacing;
    for (var i = 0; i < splitTitle.length; i++) {
      doc.textAlign(":" + splitTitle[i], { align: "left" }, this.pdfFormate.startXcol4Details, tempYC += this.pdfFormate.NormalSpacing);
    }
    ...
    doc.textAlign("Taxable Amount :", { align: "left" }, rightcol1, startY);
    doc.textAlign("" + printHeader.Amount, { align: "right-align" }, rightcol2, startY);
   
    startY = this.calculateThePage(startY += this.pdfFormate.NormalSpacing, doc);
    doc.textAlign("Total GST Amount :", { align: "left" }, rightcol1, startY);
    doc.textAlign("" + printHeader.AmountExcVat, { align: "right-align" }, rightcol2, startY);
   
    startY = this.calculateThePage(startY += this.pdfFormate.NormalSpacing, doc);
    doc.textAlign("Grand Total :", { align: "left" }, rightcol1, startY);
    doc.textAlign("" + printHeader.AmountIncludingVAT, { align: "right-align" }, rightcol2, startY); */
    ...
    startY = this.calculateThePage(startY += this.pdfFormate.NormalSpacing, doc);
    doc.textAlign("Name : " + printHeader.LastModifiedby, { align: "left" }, this.pdfFormate.startX, startY);
    //doc.textAlign("Name : ", { align: "left" }, this.pdfFormate.centerX, startY);

    //startY = this.calculateThePage(startY += this.pdfFormate.NormalSpacing, doc);
    //doc.textAlign("Phone : " + printHeader.LastModifiedPhone, { align: "left" }, this.pdfFormate.startX, startY);
    //doc.textAlign("Phone : ", { align: "left" }, this.pdfFormate.centerX, startY);

    //startY = this.calculateThePage(startY += this.pdfFormate.NormalSpacing, doc);
    //doc.textAlign("Email : " + printHeader.LastModifiedEmail, { align: "left" }, this.pdfFormate.startX, startY);
...
(function (API) {
  API.textAlign = function (txt, options, x, y) {
    ...
    } else if (options.align == "right-alignOnText") {
      if (x + txtWidth > this.internal.pageSize.width - 40) {
        x = this.internal.pageSize.width - 40 - txtWidth;
      } else {
        x = x;
      }
