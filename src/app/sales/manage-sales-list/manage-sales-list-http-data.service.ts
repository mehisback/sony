...
export class SalesorderListHttpDataService {
  ...
  getOrdersImport(parameter): any {
    return this.dataFromService.getServerData("shopify", "getOrdersImport", parameter)
      .map(getCustomer => {
        return getCustomer;
      });
  }

  importOrderDetails(parameter): any {
    return this.dataFromService.getServerData("Orders", "importOrderDetails", parameter)
      .map(getCustomer => {
        return getCustomer;
      });
  }
